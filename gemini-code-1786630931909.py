...
# Buscamos tickets que não estão finalizados/cancelados para verificar o status atual
while True:
    params_tickets = {
...
for ticket in tickets:
    ...
    # Condição 1: Status aguardando sprint
    match_sprint = "sprint" in status

    # Condição 2: Status aguardando com justificativa equipe de desenvolvimento + próximo do vencimento ou vencido
    ...
    if due_dt:
        diff = due_dt - hoje
        # Vencida (diff < 0) ou próxima do vencimento (nos próximos 2 dias)
...
def status_class(status):
    ...
    if "aguard" in val: return "status-info"
    return "status-info"
...
<div class="header">
    ...
    <h1 class="title">Relatório Semanal: Sprint & Desenvolvimento</h1>
    <p class="subtitle">Data de geração: <strong>{data_atual_str}</strong> | Tickets separados por analista</p>
</div>
<div class="content">
"""
...
if not tickets_por_analista:
    ...
    <div style="text-align: center; padding: 40px; color: #6b7280; font-size: 14px;">
        Nenhum ticket encontrado nos critérios de "Aguardando Sprint" ou "Aguardando (Desenvolvimento) Próximo/Vencido".
    </div>
    """
else:
...
msg["Subject"] = f"Relatório Semanal - Tickets Sprint & Desenvolvimento ({data_atual_str})"
